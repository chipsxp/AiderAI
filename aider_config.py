...
# Example usage futures append to models
if __name__ == "__main__":
    ...
    with ThreadPoolExecutor() as executor:
        ...
        for model in models:
            ...
            if api_key:
                if model == "gpt-4o":
                    futures.append(executor.submit(subprocess.run, ["aider", "--openai-api-key", api_key, "--architect", "--model", model, "--watch-files", "--browser"]))
                elif model == "sonnet":
                    futures.append(executor.submit(subprocess.run, ["aider", "--anthropic-api-key", api_key, "--editor-model", model, "--watch-files", "--browser"]))
